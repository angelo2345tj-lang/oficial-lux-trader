...
function isValidCandle(candle: Candle, isFinal: boolean): boolean {
  ...
  if (isFinal && candle.volume <= 0) return false;
  return true;
}

function isDelayedCandle(candle: Candle, timeframe: string): boolean {
  const map: Record<string, number> = {
    '1': 60_000,
    '5': 300_000,
    '15': 900_000,
  ...
  const interval = map[timeframe] ?? 60_000;
  return Date.now() - candle.timestamp > interval + 20_000;
}
...
export class BinanceStreamEngine {
  ...
  private handleMessage(data: unknown) {
    ...
    if (!isValidCandle(candle, isFinal)) {
    ...
    if (now - this.lastEmitAt < UPDATE_THROTTLE_MS) return;
    this.lastEmitAt = now;
    this.lastEmittedCandleTs = candle.timestamp;
    this.candleCallbacks.forEach((cb) => cb(candle, this.symbol, false));
    logger.debug(`Tick vivo ${this.symbol} close=${candle.close}`, 'Realtime');
  }
