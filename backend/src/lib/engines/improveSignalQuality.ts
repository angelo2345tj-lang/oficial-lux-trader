...
/** Ajusta entrada para reduzir drawdown inicial (evita candle esticado e melhora reteste). */
function refineEntryPrice(
  entry: number,
  direction: 'BUY' | 'SELL',
  candles: Candle[],
  structure: StructureAnalysis
): number {
  if (candles.length < 3 || entry <= 0) return entry;
  const last = candles[candles.length - 1];
  const range = last.high - last.low;
  const body = Math.abs(last.close - last.open);
  let refined = entry;

  if (range > 0 && body / range < 0.32) {
    refined = direction === 'BUY' ? Math.min(entry, last.close) : Math.max(entry, last.close);
  }

  const swings =
    direction === 'BUY' ? structure.swingLows : structure.swingHighs;
  const zone = swings?.length ? swings[swings.length - 1] : undefined;
  if (zone && zone > 0) {
    if (direction === 'BUY' && refined > zone * 1.0015) {
      refined = Math.max(zone, last.close * 0.9997);
    }
    if (direction === 'SELL' && refined < zone * 0.9985) {
      refined = Math.min(zone, last.close * 1.0003);
    }
  }

  return parseFloat(refined.toFixed(5));
}
...
export function improveSignalQuality(input: SignalQualityInput): TradeSignal {
  ...
  const rsiExhausted =
    (direction === 'BUY' && rsiVal > 72) || (direction === 'SELL' && rsiVal < 28);
  if (rsiExhausted) {
    adjusted -= 12;
    negatives.push('RSI em exaustão — evitar topo/fundo');
  }

  const last = candles[candles.length - 1];
  const range = last.high - last.low;
  const body = Math.abs(last.close - last.open);
  if (range > 0 && body / range < 0.28) {
    adjusted -= 8;
    negatives.push('Candle esticado — entrada ajustada');
  }
  ...
  const refinedEntry = refineEntryPrice(signal.entry, direction, candles, structure);
  const entryShift = refinedEntry - signal.entry;
  const tp1 = signal.tp1 + entryShift;
  const tp2 = signal.tp2 + entryShift;
  const tp3 = signal.tp3 + entryShift;
  const sl = signal.sl + entryShift;
  ...
  return {
    ...
    signalQuality: displayConfidence,
    winProbability: displayConfidence,
    confirmationStatus:
    ...
    confidenceLabel: confidenceFromScore(displayConfidence),
    ...
    mainReason: `${confidenceFromScore(displayConfidence)} · ${displayConfidence}%`,
    verdict: `${confidenceFromScore(displayConfidence)} ${displayConfidence}%`,
  };
}
