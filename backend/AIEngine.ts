...
export function localAIValidate(ctx: AIContext): AIValidation {
  ...
  const hardBlock =
    (candleAnalysis?.fakeout && winProb < 52) ||
    (isExhaustion(rsiValue, direction) && winProb < 48);

  const approved = !hardBlock && winProb >= 48;
  ...
  return {
    ...
    reason: approved
      ? `IA Operacional: ${winProb}% — setup institucional validado`
      : `IA Operacional: ${winProb}% — filtros de qualidade não atendidos`,
    source: 'local',
    adjustments,
  };
}
